...
const Chart3 = () => {
    ...
    return (
        <AnimateSharedLayout>
            {
                expanded? <ExpandedCard3 setExpanded={()=>setExpanded(false)}/> : <CompactCard3 setExpanded={()=>setExpanded(true)}/>
            }
        </AnimateSharedLayout>
    )
}

function CompactCard3 ({setExpanded}) {
    return (
...
function ExpandedCard3 ({setExpanded}) {
  ...
  .then(res => {
    ...
    for(const dataObj of res.data){
      ...
      Precip.push(Math.round(dataObj.rain_mm*100)/100)
    }
    setCategory(timearr);
    setData(Precip);
  })
